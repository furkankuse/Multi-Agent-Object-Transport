class Environment:
    ...
    def nextState(self, state, agent1action, agent2action):
        ...
        nextStateAgent1 = self.generateNextState([currentStateAgent1[0], currentStateAgent1[1]], agent1action)
        currentStateAgent2 = state.getAgent2
        nextStateAgent2 = self.generateNextState([currentStateAgent2[0], currentStateAgent2[1]], agent2action)
        ...
        # Case which they move separately
            ...
            if not agent1IsItOkay:
                nextStateAgent1 = currentStateAgent1
            if not agent2IsItOkay:
                nextStateAgent2 = currentStateAgent2
    ...
    def generateNextState(self, currentState, action):
        ...
        if action == 0 and currentState[1] - 1 >= 0:
            nextState[1] = nextState[1] - 1
        elif action == 1 and currentState[0] + 1 < len(self._env):
            nextState[0] = currentState[0] + 1
        elif action == 2 and currentState[1] + 1 < len(self._env[0]):
            nextState[1] = currentState[1] + 1
        elif action == 3 and currentState[0] - 1 >= 0:
            nextState[0] = currentState[0] - 1
        else:
            nextState = [currentState[0], currentState[1]]

        nextState.append(self.nextCond(nextState))
        return nextState
